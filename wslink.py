...
class SMS323Automation:
    ...
    def save_settings(self):
        settings = {'withdraw_platform_id': self.withdraw_platform_id}
    ...
    async def forward_to_group(self, context, message, user_info=""):
        try:
            ...
            await context.bot.send_message(chat_id=self.forward_group_id, text=formatted_message)
    ...
    def manage_websites(self, action=None, data=None, user_id=None):
        ...
        if action == "list":
            ...
            new_website = {
                "name": name, "base_url": base_url, "origin": origin,
                "referer": referer, "platform_id": platform_id
            }
            ...
            if response.status_code == 200:
                ...
                if result.get('code') == 1:
                    ...
                    for bank in banks:
                        if (bank.get('bank_name') == 'GOMONEY' and bank.get('bank_card') == '8504484734'):
                            bank_id = bank.get('id')
                            message += f"Using GOMONEY Bank: {bank.get('bank_name')} - ID: {bank_id}\n"
                            return bank_id, message
                    message += "GOMONEY bank not found\n"
                    return None, message
    ...
    def submit_withdraw(self, bank_id, amount, username, user_id):
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                if result.get('code') == 1:
                    order_data = result.get('data') or {}          # এখানে ফিক্স করলাম
    ...
    def check_withdraw_status(self, username, user_id):
        ...
        data = {'page': 1, 'size': 50, 'status': 0, 'type': 0, 'time': 0}
        try:
    ...
    async def process_all_accounts(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        self.processing_results[user_id] = {
            'successful': 0, 'total': len(accounts), 'start_time': time.time(),
            'failed_accounts': [], 'current_page': 1, 'accounts_per_page': 20
        }
    ...
    async def check_all_status(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        user = update.callback_query.from_user
        user_info = f"User: {user.first_name} {user.last_name or ''} (@{user.username or 'N/A'})"
        self.failed_withdraws[user_id] = {'failed_list': [], 'current_page': 1, 'accounts_per_page': 20}
            ...
            if "FAILED ORDERS:" in line:
                ...
                try:
                    ...
                    if amount and order_no:
                        failed_withdraw = {
                            'username': username, 'bank_name': bank_name, 'amount': amount,
    ...
    async def send_failed_withdraws_page(self, context, chat_id, user_id, page):
        ...
        if current_page_withdraws:
            keyboard.append([InlineKeyboardButton("Re-submit ALL Failed Withdraws", callback_data=f"resubmit_all_page_{page}")])
        nav_buttons = []
        if page > 1:
...
async def show_website_list(query, context, user_id):
    ...
    keyboard = []
    for i, website in enumerate(websites, 1):
        keyboard.append([InlineKeyboardButton(f"{i}. {website['name']}", callback_data=f"select_website_{i}")])
