...
class SMS323Automation:
    ...
    def check_withdraw_status(self, username, user_id):
        ...
        # Then check actual API status to update our records
        data = {
            'page': 1,
            'size': 50,
            'status': 0,
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('code') == 1:
                    ...
                    # Update our local status with API data - ONLY FOR BOT ORDERS
                    updated_orders = self.update_bot_orders_from_api(user_orders, api_orders)
    ...
    def update_bot_orders_from_api(self, user_orders, api_orders):
        """Update ONLY bot orders with actual API status"""
        updated_orders = []
        
        # Only process orders that are from bot (stored in our system)
        for user_order in user_orders:
            user_order_id = user_order.get('order_id', '')
            user_amount = user_order.get('amount', 0)
            
            # Look for matching order in API
            matching_api_order = None
            for api_order in api_orders:
                api_order_id = api_order.get('id')
                api_amount = api_order.get('score', 0)
                
                # Match by order ID or by amount and bank name
                if (user_order_id and user_order_id == api_order_id) or \
                   (user_amount == api_amount and api_order.get('bank_name') == 'GOMONEY'):
                    matching_api_order = api_order
                    break
            
            if matching_api_order:
                # Update with API data
                api_status = matching_api_order.get('status')
                status_map = {2: 'success', 1: 'pending', 3: 'failed'}
                mapped_status = status_map.get(api_status, 'pending')
                
                updated_order = {
                    "order_id": matching_api_order.get('id', user_order_id),
                    "order_no": matching_api_order.get('order_no', user_order.get('order_no', '')),
                    "amount": user_amount,
                    "bank_name": "GOMONEY",
                    "date": matching_api_order.get('createtime2', user_order.get('date', '')),
                    "status": mapped_status
                }
                updated_orders.append(updated_order)
            else:
                # Keep the original order if no API match found
                updated_orders.append(user_order)
